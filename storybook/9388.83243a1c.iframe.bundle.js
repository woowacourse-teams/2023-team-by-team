...
`,DaysOfWeek=styled_components_browser_esm.I4.div`
  ...
  height: 24px;
`,ScheduleCircleWrapper=styled_components_browser_esm.I4.div`
  display: flex;
  justify-content: center;
...
`,DateView=styled_components_browser_esm.I4.div`
  ...
  background-color: ${({theme})=>theme.color.WHITE};
`,dayOfWeek=styled_components_browser_esm.AH`
  display: flex;
  justify-content: center;
...
`,monthButton=styled_components_browser_esm.AH`
  padding: 0;
  margin-top: 4px;
`;var DateCell=__webpack_require__("./src/components/common/DateCell/DateCell.tsx"),svg=__webpack_require__("./src/assets/svg/index.ts"),constants_calendar=__webpack_require__("./src/constants/calendar.ts"),useQuery=__webpack_require__("./node_modules/@tanstack/react-query/build/lib/useQuery.mjs"),schedule=__webpack_require__("./src/apis/schedule.ts"),query=__webpack_require__("./src/constants/query.ts");var parseDate=__webpack_require__("./src/utils/parseDate.ts"),getDateByPosition=__webpack_require__("./src/utils/getDateByPosition.ts"),arrayOf=__webpack_require__("./src/utils/arrayOf.ts");const removeDuplicatesInMatrix=scheduleCirclesMatrix=>scheduleCirclesMatrix.map((scheduleCircles=>scheduleCircles.map((({teamPlaceIds})=>({teamPlaceIds:[...new Set(teamPlaceIds)]}))))),getSortedScheduleCirclesMatrix=scheduleCirclesMatrix=>scheduleCirclesMatrix.map((scheduleCircles=>scheduleCircles.map((({teamPlaceIds})=>({teamPlaceIds:[...teamPlaceIds.sort(((a,b)=>a-b))]}))))),getSlicedScheduleCirclesMatrix=scheduleCirclesMatrix=>scheduleCirclesMatrix.map((scheduleCircles=>scheduleCircles.map((({teamPlaceIds})=>({teamPlaceIds:teamPlaceIds.slice(0,constants_calendar.Q7)}))))),isDateInPeriod=(date,schedule)=>{const{startDateTime,endDateTime}=schedule,startDateBoundary=new Date(`${startDateTime.split(" ")[0]} 00:00`),endDateBoundary=new Date(`${endDateTime.split(" ")[0]} 23:59`);return date>=startDateBoundary&&date<=endDateBoundary};var TeamBadge=__webpack_require__("./src/components/team/TeamBadge/TeamBadge.tsx"),getInfoByTeamPlaceId=__webpack_require__("./src/utils/getInfoByTeamPlaceId.ts"),useTeamPlace=__webpack_require__("./src/hooks/useTeamPlace.ts"),QueryClientProvider=__webpack_require__("./node_modules/@tanstack/react-query/build/lib/QueryClientProvider.mjs");const usePrefetchMySchedules=async dateRange=>{const{startDate,endDate}=dateRange,queryKey=["mySchedules",startDate,endDate],queryClient=(0,QueryClientProvider.jE)();await queryClient.prefetchQuery(queryKey,(()=>(0,schedule.Xd)(startDate,endDate)),{staleTime:query.S.MY_SCHEDULES})};var generateCalendarRangeByYearMonth=__webpack_require__("./src/utils/generateCalendarRangeByYearMonth.ts"),jsx_runtime=__webpack_require__("./node_modules/react/jsx-runtime.js");const MyCalendar=props=>{const{onDailyClick}=props,{year,month,calendar,today,handlers:{handlePrevButtonClick,handleNextButtonClick}}=(0,useCalendar._)(),{teamPlaces}=(0,useTeamPlace.Y)(),prevCalendarYear=0===month?year-1:year,prevCalendarMonth=0===month?11:month-1,nextCalendarYear=11===month?year+1:year,nextCalendarMonth=11===month?0:month+1,schedules=(dateRange=>{const{startDate,endDate}=dateRange,{data}=(0,useQuery.I)(["mySchedules",startDate,endDate],(()=>(0,schedule.Xd)(startDate,endDate)),{staleTime:query.S.MY_SCHEDULES});if(void 0===data)return[];const{schedules}=data;return schedules})((0,generateCalendarRangeByYearMonth.Z)(year,month));usePrefetchMySchedules((0,generateCalendarRangeByYearMonth.Z)(prevCalendarYear,prevCalendarMonth)),usePrefetchMySchedules((0,generateCalendarRangeByYearMonth.Z)(nextCalendarYear,nextCalendarMonth));const scheduleCircles=((year,month,schedules)=>{const scheduleCirclesMatrix=(0,arrayOf.j)(constants_calendar.T4.ROW_SIZE).map((()=>(0,arrayOf.j)(constants_calendar.T4.COLUMN_SIZE).map((()=>({teamPlaceIds:[]})))));(0,arrayOf.j)(constants_calendar.T4.ROW_SIZE).forEach((row=>{(0,arrayOf.j)(constants_calendar.T4.COLUMN_SIZE).forEach((column=>{const currentDate=(0,getDateByPosition.L)(year,month,row,column);schedules.forEach((schedule=>{isDateInPeriod(currentDate,schedule)&&scheduleCirclesMatrix[row][column].teamPlaceIds.push(schedule.teamPlaceId)}))}))}));const duplicateRemovedCirclesMatrix=removeDuplicatesInMatrix(scheduleCirclesMatrix),sortedScheduleCirclesMatrix=getSortedScheduleCirclesMatrix(duplicateRemovedCirclesMatrix);return getSlicedScheduleCirclesMatrix(sortedScheduleCirclesMatrix)})(year,month,schedules);return(0,jsx_runtime.jsxs)(Container,{children:[(0,jsx_runtime.jsxs)(CalendarHeader,{children:[(0,jsx_runtime.jsx)(Button.A,{variant:"plain",size:"sm",onClick:handlePrevButtonClick,css:monthButton,"aria-label":"이전 달로 이동하기",children:(0,jsx_runtime.jsx)(svg.ArrowLeftIcon,{})}),(0,jsx_runtime.jsx)("time",{children:(0,jsx_runtime.jsxs)(Text.A,{size:"lg",weight:"semiBold",children:[year,"년 ",month+1,"월"]})}),(0,jsx_runtime.jsx)(Button.A,{variant:"plain",size:"sm",onClick:handleNextButtonClick,css:monthButton,"aria-label":"다음 달로 이동하기",children:(0,jsx_runtime.jsx)(svg.ArrowRightIcon,{})})]}),(0,jsx_runtime.jsxs)("div",{children:[(0,jsx_runtime.jsx)(DaysOfWeek,{children:constants_calendar.j_.map((day=>(0,jsx_runtime.jsx)(Text.A,{size:"sm",weight:"bold",css:dayOfWeek,children:day},day)))}),(0,jsx_runtime.jsx)("div",{children:calendar.map(((week,rowIndex)=>(0,jsx_runtime.jsx)(react.Fragment,{children:(0,jsx_runtime.jsx)(DateView,{children:week.map(((day,colIndex)=>{const{year:renderYear,month:renderMonth,date:renderDate}=(0,parseDate._)(day),isToday=today.year===renderYear&&today.month===renderMonth&&today.date===renderDate;return(0,jsx_runtime.jsxs)("div",{children:[(0,jsx_runtime.jsx)(DateCell.A,{rawDate:day,currentMonth:month,isToday,size:"sm",onDayClick:()=>onDailyClick(day)}),(0,jsx_runtime.jsx)(ScheduleCircleWrapper,{children:scheduleCircles[rowIndex][colIndex].teamPlaceIds.map((teamPlaceId=>{const{teamPlaceColor}=(0,getInfoByTeamPlaceId.D)(teamPlaces,teamPlaceId);return(0,jsx_runtime.jsx)(TeamBadge.A,{size:"sm",teamPlaceColor},`${day.toISOString()}+${teamPlaceId}`)}))})]},day.toISOString())}))})},rowIndex)))})]})]})},MyCalendar_MyCalendar=MyCalendar;try{MyCalendar.displayName="MyCalendar",MyCalendar.__docgenInfo={description:"",displayName:"MyCalendar",props:{onDailyClick:{defaultValue:null,description:"",name:"onDailyClick",required:!0,type:{name:"(date: Date) => void"}}}},"undefined"!=typeof STORYBOOK_REACT_CLASSES&&(STORYBOOK_REACT_CLASSES["src/components/my_calendar/MyCalendar/MyCalendar.tsx#MyCalendar"]={docgenInfo:MyCalendar.__docgenInfo,name:"MyCalendar",path:"src/components/my_calendar/MyCalendar/MyCalendar.tsx#MyCalendar"})}catch(__react_docgen_typescript_loader_error){}},"./src/components/team/TeamBadge/TeamBadge.tsx":(__unused_webpack_module,__webpack_exports__,__webpack_require__)=>{__webpack_require__.d(__webpack_exports__,{A:()=>TeamBadge_TeamBadge});var styled_components_browser_esm=__webpack_require__("./node_modules/styled-components/dist/styled-components.browser.esm.js");const Wrapper=styled_components_browser_esm.I4.div.withConfig({shouldForwardProp:prop=>!["teamPlaceColor","size"].includes(prop)})`
  ${({size})=>"sm"===size?styled_components_browser_esm.AH`
        width: 6px;
        height: 6px;
